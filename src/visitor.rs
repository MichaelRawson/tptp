...
pub trait Visitor<'a> {
    ...
    fn visit_thf_defined_infix(
        &mut self,
        thf_defined_infix: ThfDefinedInfix<'a>,
    ) {
        self.visit_thf_unitary_term(thf_defined_infix.left);
        self.visit_defined_infix_pred(thf_defined_infix.op);
    ...
    fn visit_thf_defined_atomic(
        &mut self,
        thf_defined_atomic: ThfDefinedAtomic<'a>,
    ) {
        match thf_defined_atomic {
            ThfDefinedAtomic::Constant(constant) => {
                self.visit_defined_constant(constant)
            }
        }
    }

    fn visit_thf_atomic_formula(
        &mut self,
        thf_atomic_formula: ThfAtomicFormula<'a>,
    ) {
        match thf_atomic_formula {
            ThfAtomicFormula::Plain(plain) => {
                self.visit_thf_plain_atomic(plain)
            }
            ThfAtomicFormula::Defined(defined) => {
                self.visit_thf_defined_atomic(defined)
            }
        }
    }
    ...
    fn visit_thf_unitary_formula(
        ...
        match thf_unitary_formula {
            ...
            ThfUnitaryFormula::Variable(variable) => {
                self.visit_variable(variable)
            }
    ...
    fn visit_thf_infix_unary(&mut self, thf_infix_unary: ThfInfixUnary<'a>) {
        self.visit_thf_unitary_term(thf_infix_unary.left);
        self.visit_infix_inequality(thf_infix_unary.op);
        self.visit_thf_unitary_term(thf_infix_unary.right);
    }

    fn visit_thf_unary_formula(
        &mut self,
        thf_unary_formula: ThfUnaryFormula<'a>,
    ) {
        match thf_unary_formula {
            ThfUnaryFormula::Infix(infix) => self.visit_thf_infix_unary(infix),
        }
    }
    ...
    fn visit_thf_or_formula(&mut self, thf_or_formula: ThfOrFormula<'a>) {
        for formula in thf_or_formula.0 {
            self.visit_thf_unit_formula(formula)
        }
    }

    fn visit_thf_apply_formula(
        &mut self,
        thf_apply_formula: ThfApplyFormula<'a>,
    ) {
        for formula in thf_apply_formula.0 {
            self.visit_thf_unit_formula(formula)
        }
    }
    ...
    fn visit_thf_binary_formula(
        &mut self,
        thf_binary_formula: ThfBinaryFormula<'a>,
    ) {
        match thf_binary_formula {
            ThfBinaryFormula::Assoc(assoc) => {
                self.visit_thf_binary_assoc(assoc)
            }
            ThfBinaryFormula::Type(typ) => self.visit_thf_binary_type(typ),
        }
    }

    fn visit_thf_logic_formula(
        &mut self,
        thf_logic_formula: ThfLogicFormula<'a>,
    ) {
        match thf_logic_formula {
            ThfLogicFormula::Unitary(unitary) => {
                self.visit_thf_unitary_formula(*unitary)
            }
            ThfLogicFormula::Unary(unary) => {
                self.visit_thf_unary_formula(*unary)
            }
            ThfLogicFormula::Binary(binary) => {
                self.visit_thf_binary_formula(binary)
            }
            ThfLogicFormula::DefinedInfix(defined_infix) => {
                self.visit_thf_defined_infix(*defined_infix)
            }
        }
    }
