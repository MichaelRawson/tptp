...
//! Naturally, `TPTPIterator` handles whitespace and comments anywhere.
//! 
//! ## Streaming
...
//!
//! ## Fast Failure
//! `nom` has a `cut` combinator that prevents backtracking in some cases where an error can be detected early, which is used where possible for performance.
//! This may produce surprising (but hopefully not buggy) results in rare cases.
//! 
//! ## `#![no_std]`
